...
public class TodoApp {
    ...
    public CacheManager cacheManager(final ConversionService conversionService) {
        ...
        configuration.setCachePrefix("todos");
        configuration.setTimeToLive(10);
        configuration.setServerGroupRetry(true);
        return new EVCacheManager(Collections.singletonList(configuration), conversionService); # <1>
    }
}

@Repository
public class TodoRepository {
    @Cacheable(cacheNames = "TODO.todos", key = "'findAll'") # <3>
...
<3> `cacheNames` is sames as `appName + . + cachePrefix`
...
<2> `cacheNames` is sames as `evcache.clusters[].appName + . + evcache.clusters[].cachePrefix`
...
| evcache.clusters[].allowNullValues
| true
| Whether to allow for `null` values

| evcache.clusters[].serverGroupRetry
| true
| Retry across Server Group for cache misses and exceptions

| evcache.clusters[].enableExceptionThrowing
| false
