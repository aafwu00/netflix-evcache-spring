...
package com.github.aafwu00.evcache.sidecar.spring.cloud;
 ...
 */
@Configuration
@ConditionalOnProperty(value = "evcache.sidecar.enabled", matchIfMissing = true)
@AutoConfigureAfter(EurekaClientAutoConfiguration.class)
@EnableConfigurationProperties(EVCacheSidecarProperties.class)
public class EVCacheSidecarAutoConfiguration {
    private final ConfigurableEnvironment env;
    private final EurekaInstanceConfigBean eurekaInstanceConfigBean;
    private final EVCacheSidecarProperties properties;

    public EVCacheSidecarAutoConfiguration(final ConfigurableEnvironment env,
                                           final EurekaInstanceConfigBean eurekaInstanceConfigBean,
                                           final EVCacheSidecarProperties properties) {
    ...
    @Bean
    public HasFeatures evcacheSidecarFeature() {
        return HasFeatures.namedFeature("Netflix EVCache Sidecar", EVCacheSidecarAutoConfiguration.class);
    }
    ...
    private void putIfAbsent(final Map<String, String> metadataMap, final String key, final String defaultValue) {
        metadataMap.putIfAbsent(key, env.getProperty(key, defaultValue));
    }

    private void putIfAbsentWhenContainProperty(final Map<String, String> metadataMap, final String key) {
        if (env.containsProperty(key)) {
            metadataMap.putIfAbsent(key, env.getProperty(key));
        }
    }
}
